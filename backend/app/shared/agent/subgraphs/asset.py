...
from app.shared.agent.tools.balance import get_total_balance
...
- balance: 잔액 조회 ("잔액 얼마야", "돈 얼마 있어")
- history: 거래 내역 조회 ("거래내역", "최근 내역", "이번달 내역")
- spending_analysis: 지출 분석 리포트 ("지출 분석", "소비 분석", "리포트")

다음 JSON만 반환해. 설명 없이:
{"action": "<balance|history|spending_analysis>", "period": "<이번달|지난달|3개월>", "category": "<카테고리명|null>"}
...
def _parse_llm_action(raw: str) -> dict[str, str | None]:
    ...
    if match:
        try:
            ...
            pass
    return {"action": "balance", "period": "이번달", "category": None}
...
async def asset_node(state: VoiceState) -> dict:
    ...
    if fast == "balance":
        action, period, category = "balance", None, None
    elif fast is not None:
        action, period, category = fast, prev_period, None
    else:
        llm = _get_llm()
    ...
    # ── 2. tool 실행 ─────────────────────────────────────────────────────────────
    days = _PERIOD_TO_DAYS.get(period or "이번달", 30)

    if action == "balance":
        tts_result = get_total_balance.invoke({"user_id": user_id})
    elif action == "spending_analysis":
        tts_result = get_monthly_spending_report.invoke(
            {"user_id": user_id, "period": period or "이번달"}
        )
    elif category:
        tts_result = get_category_history.invoke(
            {"user_id": user_id, "category": category, "days": days}
        )
    else:
        tts_result = get_recent_history.invoke({"user_id": user_id, "days": days})

    # ── 3. navigate_to 결정 ───────────────────────────────────────────────────────
    navigate_to = _NAVIGATE_MAP.get(action, "balance")
    assert navigate_to in ASSET_NAVIGATE_VALUES, f"계약 위반: {navigate_to}"
