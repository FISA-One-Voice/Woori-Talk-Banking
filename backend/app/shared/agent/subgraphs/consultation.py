...
async def rag_graph(state: VoiceState) -> dict:
    ...
    RAG 에이전트 본체를 실행한 뒤, 이벤트 조회(get_event_list) 툴이 사용되었다면
    화면 이동(navigate_to="event")을 주입하고, 그 외의 경우 화면 이동 권한이 없음을 
    보장하기 위해 `navigate_to: None`을 반환합니다.
    """
    result = await rag_agent.ainvoke(state)

    # ToolMessage 중 이벤트 조회 툴이 사용되었는지 확인
    nav = None
    for msg in result["messages"]:
        if getattr(msg, "name", None) == "get_event_list":
            nav = "event"
            break
