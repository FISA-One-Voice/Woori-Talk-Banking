"""화면 담당자가 실제 tool을 등록하기 전까지 사용하는 mock 구현체 (Issue #21, #48).
...
담당자별 교체 대상:
    asset    담당자 (A): mock_query_asset → features/asset/tools (Issue #48)
...
def mock_query_asset(
    ...
    "잔액 얼마야", "이번달 식비 얼마야", "최근 거래 내역" 등의 말에 사용합니다.
    ...
    if action == "category":
        cat = category or "기타"
        label = period or "최근"
        return f"{label} {cat} 지출은 총 삼건, 사만 오천 원입니다."
...
def mock_get_balance(user_id: str) -> str:
    ...
    '잔액 얼마야', '돈 얼마 있어', '잔액 알려줘' 등의 말에 사용합니다.
...
def mock_get_history(user_id: str, days: int = 7, category: str = "") -> str:
    ...
    '최근 거래 내역', '이번 달 지출', '소비 내역' 등의 말에 사용합니다.
...
@tool
def mock_execute_transfer(user_id: str, recipient: str, amount: int) -> str:
    """등록된 수취인에게 금액을 이체합니다.
    ...
    Args:
        user_id: 현재 로그인한 사용자 ID.
        recipient: 수취인 이름 (resolve_node 정규화 값).
        amount: 이체 금액 (원 단위).
...
def mock_register_auto_transfer(
    ...
    Args:
        ...
        recipient: 수취인 이름.
        amount: 이체 금액 (원 단위).
        cycle: 주기 ("monthly" 또는 "weekly").
        scheduled_day: 이체일 (1~31).
...
def mock_lookup_recipient(user_id: str, recipient: str) -> str | None:
    ...
    Args:
        user_id: 요청 사용자 ID.
        recipient: recipient 슬롯 값 (이름, 별명).
...
def mock_get_events(user_id: str) -> str:
    ...
    '이벤트 뭐 있어', '행사 알려줘', '이번 달 이벤트' 등의 말에 사용합니다.
