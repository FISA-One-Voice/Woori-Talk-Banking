...
def _normalize_period(period: str | None) -> str | None:
    """STT 오인식 보정 — '최근 7일', '최근칠일', '7일' 등을 '최근7일'로 정규화."""
    if not period:
        return period
    p = period.replace(" ", "").replace("일", "일")
...
def _query_balance(db: Session, user_id: str) -> str:
...
def _query_transaction_list(db: Session, user_id: str, period: str | None, date_range: str | None) -> str:
...
def _query_history(
    db: Session, user_id: str, period: str | None, date_range: str | None, filter_type: str | None = None
) -> str:
...
def _query_category(db: Session, user_id: str, period: str | None, category: str | None) -> str:
...
def _query_top_category(db: Session, user_id: str, period: str | None) -> str:
...
def query_asset(
    ...
    try:
        ...
        if action == "transaction_list":
            return _query_transaction_list(db, user_id, period, date_range)
        if action == "history":
            return _query_history(db, user_id, period, date_range, filter_type)
        if action == "category":
