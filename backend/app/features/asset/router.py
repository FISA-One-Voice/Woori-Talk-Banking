...
# GET /api/asset/compare              → 두 기간 지출 비교
# =============================================================================
...
from app.features.asset.service import build_summary_tts, build_transaction_tts, get_compare_data
...
def get_expense_compare(
    ...
    Args:
        period: 기준 기간. "이번달"|"지난달"|"최근7일". 기본값 "이번달".
        compare_period: 비교 기간. "이번달"|"지난달"|"최근7일". 기본값 "지난달".
        category: 카테고리 필터. None이면 전체 지출 비교.
        db: 데이터베이스 세션.
        user_id: JWT에서 추출한 인증 사용자 ID.

    Returns:
        period, compare_period, period_amount, compare_amount, diff 포함 응답.
    """
    data = get_compare_data(db, user_id, period, compare_period, category)
    return {
        "success": True,
        "data": data,
        "message": f"{period} vs {compare_period} 지출 비교입니다.",
    }
