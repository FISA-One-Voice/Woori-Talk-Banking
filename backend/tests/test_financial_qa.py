import pytest
from app.core.opensearch import create_indices_if_not_exists, get_os_client, FINANCIAL_DOCS_INDEX
...
def test_search_financial_docs_empty_real():
    ...
    result = search_financial_docs.invoke({"query": "절대존재하지않는외계인은행", "user_id": "test_user"})
...
def test_search_financial_docs_error_real():
    ...
    with pytest.raises(OpenSearchError) as exc_info:
        search_financial_docs.invoke({"query": "에러 발생 테스트", "user_id": "test_user"})
