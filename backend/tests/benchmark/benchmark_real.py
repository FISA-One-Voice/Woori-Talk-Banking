"""LLM 모델 벤치마크 — 실제 DB 연결 버전.

실제 사용자(BENCH_USER_ID)로 DB를 조회하여 에이전트 전체 흐름을 측정합니다.
mock 버전과 비교하면 DB 조회 오버헤드를 파악할 수 있습니다.

실행:
    cd backend
    .venv/bin/python tests/benchmark/benchmark_real.py

주의:
    - Aiven PostgreSQL 연결 필요 (.env POSTGRES_* 설정)
    - 해당 사용자의 실제 잔액/수취인 데이터를 조회합니다.
    - 이체 확인 단계(awaiting_confirmation)까지만 진행하며 실제 이체는 실행하지 않습니다.
"""
...
REPEAT = 10
BENCH_USER_ID = "262e5403-30f3-4807-86fa-f7d4ff93990e"
...
def _make_scenarios(recipient_alias: str) -> list[Scenario]:
    return [
        # ── 임시 비활성화 ──────────────────────────────────────────────────────
        # Scenario(
        #     name="잔액조회",
        #     utterance="내 통장 잔액 알려줘",
        #     expected_keywords=["잔액", "원"],
        # ),
        # Scenario(
        #     name="이체_풀슬롯",
        #     utterance=f"{recipient_alias}에게 만원 이체해줘",
        #     expected_keywords=[recipient_alias, "이체", "확인", "만원", "10,000"],
        ...
        Scenario(
            name="이체_드려줘",
            utterance=f"{recipient_alias}한테 만원 드려줘",
            expected_keywords=[recipient_alias, "이체", "확인", "만원", "10,000"],
            expects_slot_collection=True,
        ),
        Scenario(
            name="이체_드릴까요",
            utterance=f"{recipient_alias}한테 만원 드릴까요?",
            expected_keywords=[recipient_alias, "이체", "확인", "만원", "10,000"],
            expects_slot_collection=True,
        ),
        Scenario(
            name="이체_전달해줘",
            utterance=f"{recipient_alias}한테 만원 전달해줘",
            expected_keywords=[recipient_alias, "이체", "확인", "만원", "10,000"],
            expects_slot_collection=True,
        ),
    ]
...
def _lookup_recipient_alias() -> str:
    """DB에서 테스트 유저의 첫 번째 수취인 alias를 조회한다."""
    session = SessionLocal()
    try:
        row = (
            session.query(RegisteredRecipient)
            .filter(RegisteredRecipient.user_id == uuid.UUID(BENCH_USER_ID))
            .first()
        )
        alias = row.alias if row else "테스트수취인"
        print(f"  [DB] 테스트 수취인 alias: '{alias}'")
        return alias
    finally:
        session.close()
...
async def _run_model(model: str, scenarios: list[Scenario]) -> list[Scenario]:
    """시나리오 목록은 모델마다 fresh 복사본을 받는다."""
...
def _print_and_save_summary(
    all_results: dict[str, list[Scenario]], recipient_alias: str
) -> None:
    ...
    print(f"\n\n{'=' * total_w}")
    print(f"  [REAL] LLM 벤치마크 요약  repeat={REPEAT}")
    print(f"  user_id={BENCH_USER_ID}  alias='{recipient_alias}'")
    print(f"{'=' * total_w}")
    ...
    report_path = REPORT_DIR / f"benchmark_real_all_{ts}.md"
    ...
    content = f"""# LLM 벤치마크 결과 [REAL]

- **모델 비교**: {", ".join(models)}
- **반복**: {REPEAT}회
- **유저**: {BENCH_USER_ID}
- **수취인 alias**: {recipient_alias}
...
async def main() -> None:
    recipient_alias = _lookup_recipient_alias()

    all_results: dict[str, list[Scenario]] = {}
    for model in MODELS:
        # 모델마다 독립된 Scenario 인스턴스 생성
        scenarios = _make_scenarios(recipient_alias)
        all_results[model] = await _run_model(model, scenarios)

    _print_and_save_summary(all_results, recipient_alias)
