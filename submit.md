...
**CI/CD 및 배포**  
GitHub Actions 기반 CI/CD 파이프라인을 통해 코드 변경 시 자동으로 빌드 및 배포가 이루어집니다. Backend(FastAPI)는 코드 변경 시 자동으로 도커 빌드 및 배포가 이루어집니다.

**데이터 및 보안**  
PostgreSQL, S3, OpenSearch와 연동하여 금융 데이터 저장 및 검색 기능을 수행합니다. 송금 등 민감 기능 수행시 ASV(화자인증)를 통해 본인 확인을 수행합니다.
...
#### [기능 2] CAM++ ASV 화자 인증 (192차원 임베딩 + 코사인 유사도)

- **파일:** [ai/asv/model.py](https://github.com/FISA-One-Voice/Woori-Talk-Banking/blob/main/ai/asv/model.py)
...
#### [기능 3] STT 오디오 입력 검증 파이프라인

- **파일:** [backend/app/shared/voice/stt_service.py](https://github.com/FISA-One-Voice/Woori-Talk-Banking/blob/main/backend/app/shared/voice/stt_service.py)
...
#### [기능 4] VoiceState 멀티턴 슬롯 수집 상태 머신

- **파일:** [backend/app/shared/agent/state.py](https://github.com/FISA-One-Voice/Woori-Talk-Banking/blob/main/backend/app/shared/agent/state.py)
