...
**CI/CD 및 배포**  
GitHub Actions 기반 CI/CD 파이프라인을 통해 코드 변경 시 자동으로 빌드 및 배포가 이루어집니다.
...
#### [기능 3] ASV 성공 후 즉시 실행하지 않는 이체 처리 구조
...
- **설명:** ASV 인증 성공 직후 실제 이체를 바로 실행하지 않고, `execution_ready=True` 상태만 저장한 뒤 `execution_pending=True` 응답을 먼저 반환. 프론트는 "처리 중" TTS를 재생한 후 `/api/voice/proceed`를 호출해 실제 이체를 실행한다. 인증 피드백, 이체 실행, 동의 음성 업로드를 분리해 사용자가 느끼는 대기 시간을 줄임
...
async def _return_processing_tts(config: dict, graph) -> VoiceResponseData:
    ...
    audio_mp3 = await synthesize_speech(
        "인증이 완료되었습니다. 이체 처리와 이체 확인 음성을 업로드 중입니다. "
        "잠시만 기다려주십시오."
    )
